...
export function renderStatisticsTable(
  ...
  Log.blue(LOG_CATEGORIES.DebugShowingStatsTable)(
    "sumstats",
    plotData.sumStats
  );
  ...
  table
    ...
    .data(function (row: any) {
      ...
      return row.map(function (c: any) {
        const format = SumStatsConfig.find(
          (d: SumStatsSettings) => d.name == row[0]
        )?.format;
  ...
  return { headerRowHeight: leftMostColumnWidth, tableContainer };
}

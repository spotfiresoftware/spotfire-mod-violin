...
import { Size, Tooltip, DataViewRow, ContextMenu, GeneralStylingInfo, ScaleStylingInfo } from "spotfire-api";
...
export async function render(
    ...
    Log.green(LOG_CATEGORIES.Rendering)("Plotdata min/max", plotData.yDataDomain.min, plotData.yDataDomain.max);

    container.style("background-color", styling.generalStylingInfo.backgroundColor);
    ...
    const isTrellisWithIndividualZoom = isTrellis && config.individualZoomSlider.value();
    const isTrellisWithGlobalZoom =
        isTrellis && (!config.showZoomSliders.value() || !config.individualZoomSlider.value());
    ...
    const margin = { top: 20, bottom: isTrellis ? 40 : 15, left: calculatedLeftMargin };
    ...
    Log.green(LOG_CATEGORIES.Rendering)("Show chart size:", containerSize, width, height);
    ...
    Log.red(LOG_CATEGORIES.DebugShowAllXValues)("orderedCategories before sort:", orderedCategories);
    ...
    if (config.orderBy.value()! != "") {
        ...
        const propertyToSortSettings = SumStatsConfig.find((s: SumStatsSettings) => s.name == orderBySettings[0]);

        // Only sort if that property is enabled in the table
        if (
            !Array.from(config.GetStatisticsConfigItems().values()).some(
                (m: StatisticsConfig) => m.tableEnabled && m.name == propertyToSortSettings.name
            )
        ) {
        ...
        if (orderBySettings[1] == "ordered-right") {
            tempOrderedCategories = tempOrderedCategories.sort((a: any, b: any) =>
                d3.descending(a.value[propertyToSortSettings.property], b.value[propertyToSortSettings.property])
            );
            plotData.sumStats = new Map(
                Array.from(plotData.sumStats).sort((a, b) =>
                    d3.descending(a[1][propertyToSortSettings.property], b[1][propertyToSortSettings.property])
                )
            );
        }
        if (orderBySettings[1] == "ordered-left") {
            tempOrderedCategories = tempOrderedCategories.sort((a: any, b: any) =>
                d3.ascending(a.value[propertyToSortSettings.property], b.value[propertyToSortSettings.property])
            );
            plotData.sumStats = new Map(
                Array.from(plotData.sumStats).sort((a, b) =>
                    // a[0], b[0] is the key of the map
                    d3.ascending(a[1][propertyToSortSettings.property], b[1][propertyToSortSettings.property])
                )
            );
        }
    ...
    Log.green(LOG_CATEGORIES.Rendering)(tableContainer.node().getBoundingClientRect());
    const heightAvailable = Math.max(
        0,
        height - tableContainer.node().getBoundingClientRect().height - margin.top - margin.bottom
    );
    ...
    svg.attr("style", "width:" + (width + margin.left) + "px; " + "height:" + (heightAvailable + 30) + "px;");
    ...
    if (isTrellisWithIndividualZoom) {
        Log.green(LOG_CATEGORIES.Rendering)(config.trellisIndividualZoomSettings.value());
        if (config.trellisIndividualZoomSettings.value() != "") {
            const trellisZoomConfigs: TrellisZoomConfig[] = JSON.parse(config.trellisIndividualZoomSettings.value());
            Log.green(LOG_CATEGORIES.Rendering)(trellisZoomConfigs, trellisName);
            const trellisZoomConfig = trellisZoomConfigs.find((d: TrellisZoomConfig) => d.trellisName == trellisName);
        ...
        Log.green(LOG_CATEGORIES.Rendering)("Getting min/max zoom from config if set");
        minZoom = config.yZoomMinUnset.value() ? plotData.yDataDomain.min : config.yZoomMin.value();
        maxZoom = config.yZoomMaxUnset.value() ? plotData.yDataDomain.max : config.yZoomMax.value();
    }
    ...
    let yScale = d3.scale;
    if (isScaleLog) {
        if (plotData.yDataDomain.min < LOG_Y_MIN) {
            // warn the user that the origin is small
        }
        plotData.dataPoints = plotData.dataPoints.filter((d) => d.y > LOG_Y_MIN);
        yScale = d3
            .scaleLog()
    ...
    const yAxis = d3.axisLeft(yScale).tickFormat(d3.format(config.GetYAxisFormatString()));
    ...
    const verticalSlider = sliderLeft() //!isTrellis || isTrellisWithIndividualZoom ? yScale : undefined)
        ...
        .on("end ", (val: any) => {
            ...
            if (isTrellisWithIndividualZoom) {
                ...
                Log.green(LOG_CATEGORIES.Rendering)("trellisZoomConfigs", trellisZoomConfigs);

                const trellisZoomConfig = trellisZoomConfigs?.find((d: TrellisZoomConfig) => {
                    Log.green(LOG_CATEGORIES.Rendering)(d);
                    return d.trellisName == trellisName;
                });
                Log.green(LOG_CATEGORIES.Rendering)("trellisZoomConfig", trellisZoomConfig);
                ...
                if (trellisZoomConfig != undefined) {
                    ...
                    config.trellisIndividualZoomSettings.set(JSON.stringify(trellisZoomConfigs));
                } else {
    ...
    if (!isTrellis) {
        ...
        Log.green(LOG_CATEGORIES.Rendering)("appending sliderSvg to ", svg, " for ", trellisName);
    ...
    const statisticsConfig: Map<string, StatisticsConfig> = config.GetStatisticsConfigItems();
    ...
    if (xScale.domain()[0] != "") {
        Array.from(statisticsConfig.values())
            ...
            .forEach((r: StatisticsConfig) => {
                const lineSettings = SumStatsConfig.find((e: SumStatsSettings) => e.name === r.name);

                Log.green(LOG_CATEGORIES.Rendering)("drawing trend line for: " + lineSettings.property, r);
                ...
                g.append("path")
                    ...
                    .attr(
                        ...
                        d3
                            ...
                            .y((d: any) => (!isScaleLog || d.y > LOG_Y_MIN ? yScale(d.y) : yScale(LOG_Y_MIN)))
                    );
                ...
                g.append("path")
                    ...
                    .on("mousemove", (event: MouseEvent) => {
                        tooltip.show(
                            lineSettings.name +
                                ": " +
                                d3.format(config.GetYAxisFormatString())(yScale.invert(event.clientY - margin.top))
                        );
                    })
    ...
    Array.from(statisticsConfig.values())
        ...
        .forEach((r: StatisticsConfig) => {
            const sumStatsSetting: SumStatsSettings = SumStatsConfig.find((e: SumStatsSettings) => e.name === r.name);

            Log.green(LOG_CATEGORIES.ReferenceLines)("Drawing reference line for:" + sumStatsSetting.property);
            ...
            g.selectAll(".reference-line-" + r.name)
                .data(
                    new Map(
                        [...plotData.sumStats].filter(
                            ([, v]) =>
                                v[sumStatsSetting.property] != undefined &&
                                (!isScaleLog || v[sumStatsSetting.property] > LOG_Y_MIN)
                        )
                    )
                )
                .enter()
                ...
                .attr("transform", (d: any) => {
                    ...
                    return (
                        ...
                        "," +
                        (yScale(d[1][sumStatsSetting.property]) + sumStatsSetting.verticalOffset(xScale.bandwidth())) +
                        ") rotate(" +
                ...
                .on("mouseover", function (event: d3.event, d: any) {
                    ...
                    tooltip.show(
                        d[0] +
                            ...
                            ": " +
                            d3.format(config.GetYAxisFormatString())(d[1][sumStatsSetting.property])
                    );
                })
                .on("mouseout", () => tooltip.hide());
            ...
            g.selectAll(".reference-linelabel")
                ...
                .attr("transform", function (d: any) {
                    return (
                        "translate(" +
                        ((xScale(d[0]) ? xScale(d[0]) : 0) + sumStatsSetting.labelHorizOffset(xScale.bandwidth())) +
                        "," +
    ...
    const renderedPanel: RenderedPanel = {
        ...
        mark(x, y, width, height, ctrlKey) {
            Log.green(LOG_CATEGORIES.Marking)("Render Marking panel", trellisName, x, y, width, height);
    ...
    function rectMark(
        ...
        d3.selectAll(".violin-path-markable")
            ...
            .each(function (d: d3.path, i: number, g: NodeList) {
                // If xScale.domain()
                const violinXindex = xScale.domain()[0] == "" ? 0 : xScale.domain().indexOf(d[0].category);

                //if (violinXindex > 0) return; // todo - remove. Just for debugging

                Log.green(LOG_CATEGORIES.ViolinMarking)("violinMark", d, violinXindex, xScale.domain());
                ...
                const selectionBoxStartXIndex = Math.floor((selectionBoxX - margin.left) / xScale.bandwidth());
                const selectionBoxEndXIndex = Math.floor(
                    (selectionBoxX + selectionBoxWidth - margin.left - 20) / xScale.bandwidth()
                );
                ...
                if (violinXindex >= selectionBoxStartXIndex && violinXindex <= selectionBoxEndXIndex) {
                    ...
                    if (
                        yScale.invert(selectionBoxY - margin.top) > d3.max(d.map((v: any) => v.violinY)) &&
                        yScale.invert(selectionBoxY + selectionBoxHeight - margin.top) <
                            d3.max(d.map((v: any) => v.violinY))
                    ) {
                        selectionTop = yScale(d3.max(d.map((v: any) => v.violinY))) + margin.top + 1;
                        Log.green(LOG_CATEGORIES.ViolinMarking)("violinMark adjusted selectionTop", selectionTop);
                        rectHeight = rectHeight - (selectionTop - selectionBoxY);
                    }

                    // Now bottom
                    if (
                        yScale.invert(selectionBoxY + selectionBoxHeight) < d3.min(d.map((v: any) => v.violinY)) &&
                        yScale.invert(selectionBoxY) > d3.min(d.map((v: any) => v.violinY))
                    ) {
                        rectHeight = yScale(d3.min(d.map((v: any) => v.violinY))) - selectionTop + margin.top - 1;
                        Log.green(LOG_CATEGORIES.Rendering)("violinMark adjusted rectHeight", rectHeight);
                    }
                    ...
                    if (intersections.status == "Intersection") {
                        Log.green(LOG_CATEGORIES.DebugFirefoxMarking)("violinMark intersections", intersections);
                        // Useful for debugging intersection - draws circles at the computed intersection points
                        if (DEBUG_VIOLIN_MARKING)
                            svg.selectAll(".test_points")
                                ...
                                .attr(
                                    "cx",
                                    (d: any) =>
                                        padding.violinX / 2 + margin.left + d.x + xScale.bandwidth() * violinXindex
                                )
                        ...
                        Log.green(LOG_CATEGORIES.ViolinMarking)("violinMarkables", violinMarkables);
                    }
                }
            });

        if (DEBUG_VIOLIN_MARKING)
            svg.selectAll("test_rect")
        ...
        const markingPath = svg
            .append("path")
            .attr("d", rect(selectionBoxX, selectionBoxY, selectionBoxWidth, selectionBoxHeight))
        ...
        // Marking Points
        const markedPoints = d3.selectAll(".markable-points").filter(function (d: any, i: number, g: NodeList) {
            if (isTrellis) {
                return d.trellis == trellisName && svgNode.checkIntersection(g[i], markingPath.node().getBBox());
            } else {
        ...
        if (!config.includeViolin.value()) {
            markedBoxSegments = d3.selectAll(".markable").filter(function (d: any, i: number, g: NodeList) {
                Log.blue(LOG_CATEGORIES.DebugMarkingOffset)(d, i, g);
                if (isTrellis) {
                    return (
                        d.stats.trellis == trellisName && svgNode.checkIntersection(g[i], markingPath.node().getBBox())
                    );
                } else {
                    return svgNode.checkIntersection(g[i], markingPath.node().getBBox());
                }
        ...
        Log.green(LOG_CATEGORIES.Rendering)("markedBoxSegments", markedBoxSegments?.data());
        Log.green(LOG_CATEGORIES.Rendering)(

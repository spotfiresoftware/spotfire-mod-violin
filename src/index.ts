...
import { addTrellisSettings, createSettingsPopout, removeTrellisSettings } from "./settings";
import {
    DataView,
...
export class Log {
    static green(category: LOG_CATEGORIES): (...args: any) => void {
        ...
        if (CURRENT_LOG_CATEGORIES.find((c) => c == category)) {
            return console.log.bind(console, `%c${LOG_CATEGORIES[category]}`, `background: #FFFFF; color: #31A821`);
        } else {
            return function () {};
        }
    }
    ...
    static red(category: LOG_CATEGORIES): (...args: any) => void {
        ...
        if (CURRENT_LOG_CATEGORIES.find((c) => c == category)) {
            return console.log.bind(console, `%c${LOG_CATEGORIES[category]}`, `background: #FFFFF; color: #DA5555`);
        } else {
            return function () {};
        }
    }
    ...
    static blue(category: LOG_CATEGORIES): (...args: any) => void {
        ...
        if (CURRENT_LOG_CATEGORIES.find((c) => c == category)) {
            return console.log.bind(console, `%c${LOG_CATEGORIES[category]}`, `background: #FFFFF; color: #5560DA`);
        } else {
            return function () {};
        }
    }
...
export function getComplementaryColor(backgroundColor: String) {
    ...
    if (color < 0x7fffff) {
        // lighten
        Log.green(LOG_CATEGORIES.Coloring)("adjust lighter", backgroundColor, adjustColor(backgroundColor, 0x10));
        return adjustColor(backgroundColor, 0x10);
    } else {
        // darken
        Log.green(LOG_CATEGORIES.Coloring)("adjust darker", backgroundColor, adjustColor(backgroundColor, -0x3f));
...
Spotfire.initialize(async (mod) => {
    ...
    Log.green(LOG_CATEGORIES.ShowHideZoomSliders)("is interactive in init", context.interactive);
    ...
    async function onChange(
        ...
        Log.green(LOG_CATEGORIES.General)("reloadTrigger", reloadTrigger.value());
        Log.green(LOG_CATEGORIES.ShowHideZoomSliders)("is interactive in onchange", context.interactive);
        ...
        const config: Options = {
            ...
            GetStatisticsConfigItems(): Map<string, StatisticsConfig> {
                if (!statisticsConfigCache || (statisticsConfig.value() == "" && statisticsConfigCache.size == 0)) {
            ...
            SetStatisticsConfigItem(statisticsConfigItem: StatisticsConfig) {
                ...
                Log.blue(LOG_CATEGORIES.DebugStatisticsSettings)("cache", statisticsConfigCache);
                this.GetStatisticsConfigItems().set(statisticsConfigItem.name, statisticsConfigItem);
                ...
                statisticsConfig.set(JSON.stringify(Array.from(this.GetStatisticsConfigItems().entries())));
            },
        ...
        if ((await dataView.continuousAxis("Y")) == null) {
            ...
            mod.controls.errorOverlay.show("Error: Please specify an expression for the Y axis", "yAxisMissingExpr");
        ...
        const aggregatedExpressionMatch = yAxisExpression.match("[a-zA-Z]+(\\(\\[.+\\]\\))");
        ...
        if (
            ...
            (previousYAxisExpression != yAxisExpression || ignoreAggregatedYAxisWarning.value() == false)
        ) {
            createWarning(
                reloadTrigger,
                ...
                "Remove aggregation",
                async () => {
                    (await mod.visualization.axis("Y")).setExpression(aggregatedExpressionMatch[1]);
                },
        ...
        const countAxisExpression = (await mod.visualization.axis("Count")).expression;
        ...
        if (
            ...
            (previousCountAxisExpression != countAxisExpression || ignoreIncorrectCountExpression.value() == false)
        ) {
            createWarning(
                reloadTrigger,
                context.styling.general.font,
        ...
        const trellisCount: number = (await dataView.hierarchy("Trellis")).leafCount;
        ...
        */
        ...
        Log.green(LOG_CATEGORIES.General)("checking raw property in onchange", statisticsConfig.value());
        ...
        MOD_CONTAINER.style("background-color", context.styling.general.backgroundColor);
            ...
            ? MOD_CONTAINER.append("div")
                ...
                .on("mousewheel", function (event: WheelEvent) {
                  ...
                  .on("mousedown", (e: MouseEvent) =>
                      Log.green(LOG_CATEGORIES.General)("ui marking root container mousedown", e, e.y)
                  )
                  .on("activate.bs.scrollspy", () => Log.green(LOG_CATEGORIES.General)("scrollspy"))
            : MOD_CONTAINER.select("#root-container").classed("container-fluid", true);
        //.classed("container-offset-right", true);
        ...
        function CountChildren(
            ...
            function RecursiveTrellisCount(
                ...
                let count = 0;
                Log.green(LOG_CATEGORIES.General)("count currentLevel, children", currentLevel, node.children);
                if (node.children && currentLevel < toLevel - 1) {
                    ...
                    node.children.forEach((node) => (count += RecursiveTrellisCount(node, toLevel, currentLevel + 1)));
        ...
        if (mod.getRenderContext().isEditing) {
            ...
            createSettingsPopout(config, isTrellis, document.getElementById("settings-menu"));
        } else {
            d3.selectAll(".dropdown-container").style("visibility", "hidden");
        }

        Log.green(LOG_CATEGORIES.ShowHideZoomSliders)("is interactive", isInteractive);
        ...
        if (isTrellis) {
            ...
            if (isInteractive && config.showZoomSliders.value() && !config.individualZoomSlider.value()) {
        ...
        async function TrellisedRender(
            ...
            Log.green(LOG_CATEGORIES.General)("entering trellis render");
            ...
            const rootContainerWidth = config.showZoomSliders.value()
                ? windowSize.width - globalZoomSliderContainer.node()?.getBoundingClientRect().width - 8
                : windowSize.width - 8;

            Log.green(LOG_CATEGORIES.General)(rootContainer);
            rootContainer
                .classed("root-container-trellis-with-global-zoom-slider", !config.individualZoomSlider.value())
                .classed("root-container-trellis-with-individual-zoom-slider", config.individualZoomSlider.value())
                .attr(
                    "style",
                    "width:" +
                        rootContainerWidth +
                        ...
                        "px; " +
                        "left: " +
                        (config.individualZoomSlider.value() || !config.showZoomSliders.value()
                            ? "0px"
                            : globalZoomSliderContainer.node().getBoundingClientRect().width + "px")
                );
            ...
            if (trellisPanelCount <= config.maxRowsPerPage.value() * config.maxColumnsPerPage.value()) {
                const factor = Math.sqrt(trellisPanelCount / (rowsPerPage * columnsPerPage));
                Log.green(LOG_CATEGORIES.General)(
                    "factor",
                    factor,
                    "rows * cols",
            ...
            if (rowsPerPage * (columnsPerPage - 1) >= trellisPanelCount) columnsPerPage--;
            if ((rowsPerPage - 1) * columnsPerPage >= trellisPanelCount) rowsPerPage--;

            // Just remove everything if the layout has changed
            if (rowsPerPage != previousTrellisRowsPerPage || columnsPerPage != previousTrellisColumnsPerPage) {
                rootContainer.selectAll("*").remove();
            }
            ...
            MOD_CONTAINER.attr("height", Math.ceil(panelHeight * (trellisPanelCount / columnsPerPage)));

            // Adjust panelWidth for individual zoom slider
            panelWidth = config.individualZoomSlider.value() ? panelWidth - 5 : panelWidth - 4;
            ...
            const trellisRenderingInfo: Array<Partial<TrellisRenderingInfo>> = new Array();
            ...
            function getOrCreateContainers(
                ...
                const subContainer = currentRow.select("#trellis-sub-container-" + panelIndex).empty()
                    ? currentRow
                          .append("div")
                          .attr("style", "background-color: " + context.styling.general.backgroundColor + ";")
                ...
                if (node) {
                    const titleContainer = subContainer.select("#trellis-title-container-" + panelIndex).empty()
                        ? subContainer
                              ...
                              .style("border-color", getBorderColor(context.styling.general.backgroundColor))
                    ...
                    const titleText = titleContainer
                        ...
                        .style("background-color", () => getComplementaryColor(context.styling.general.backgroundColor))
                        .style("border-style", "solid")
                        .style("border-width", "1px")
                        .style("border-color", getBorderColor(context.styling.general.backgroundColor))
                        .style("background-color", getComplementaryColor(context.styling.general.backgroundColor))
                    ...
                    bodyHeight = rowHeight - titleText.node().getBoundingClientRect().height;

                    Log.green(LOG_CATEGORIES.General)(bodyHeight);

                    const bodyContainer = subContainer.select("#trellis-body-container-" + panelIndex).empty()
                        ? subContainer
                              .append("div")
                    ...
                    const bodyContent = bodyContainer.select("#trellis-body-content-" + panelIndex).empty()
                        ? bodyContainer
                              .append("div")
            ...
            async function RecursiveTrellisedRender(
                ...
                if (node.children && currentLevel < toLevel) {
                    ...
                    node.children.forEach((node) => RecursiveTrellisedRender(node, toLevel, nextLevel, currentRow));
                } else {
                    ...
                    const { bodyContent, bodyHeight, bodyContainer } = getOrCreateContainers(
                        currentRow,
                        panelIndex,
                        node,
                        panelHeight
                    ...
                    Log.green(LOG_CATEGORIES.General)("Zoomed panel title", trellisPanelZoomedTitle);
                    Log.green(LOG_CATEGORIES.General)("incrementing panel index");
                    panelIndex++;

                    const renderedPanelWidth = bodyContainer.node().getBoundingClientRect().width as number;

                    // If we've come here as a result of a zoom slider change, then just re-render this panel only
                    if (trellisPanelZoomedTitle == "" || trellisPanelZoomedTitle == node.formattedPath()) {
                        Log.green(LOG_CATEGORIES.General)(
            ...
            while (removePanelIndex < previousTrellisPanelCount) {
                Log.green(LOG_CATEGORIES.General)("previous now panel panelIndex", removePanelIndex);
            ...
            for (const renderingInfo of trellisRenderingInfo) {
                ...
                await buildData(renderingInfo.node, dataView, config)
                    ...
                    .catch((error: Error) => {
                        Log.red(LOG_CATEGORIES.DebugDataBailout)("Caught error in buildData", error);
                        MOD_CONTAINER.selectAll("*").remove();
            ...
            // any marked records
            trellisRenderingInfo.map((renderingInfo: Partial<TrellisRenderingInfo>) => {
                if (renderingInfo.data) {
                    renderingInfo.data.isAnyMarkedRecords = trellisRenderingInfo.some(
                        (renderingInfo: Partial<TrellisRenderingInfo>) => renderingInfo.data?.isAnyMarkedRecords == true
                    );
                }
            });
            ...
            for (let i = 0; i < trellisRenderingInfo.length; i++) {
                ...
                const maxAsString = d3.format(config.GetYAxisFormatString())(renderingInfo.data.yDataDomain.max);
                const minAsString = d3.format(config.GetYAxisFormatString())(renderingInfo.data.yDataDomain.min);

                const maxStringLength = Math.max(maxAsString.length, minAsString.length);

                const calculatedLeftMargin =
                    (config.individualZoomSlider.value() ? 70 : 40) + 20 * (maxStringLength / 4);

                Log.red(LOG_CATEGORIES.DebugMarkingOffset)("calculated left margin", calculatedLeftMargin);
        ...
        if (!isTrellis) {
            buildData(await trellisAxisHierarchy.root(), dataView, config)
                .then(async (data) => {
                    ...
                    if (data == undefined) {
                        ...
                        // most likely to do with dataView expiring. For now, bail out.
                        // A refresh of the dataView should sort things out (e.g. changing a filter)
                        Log.red(LOG_CATEGORIES.DebugLatestMarking)("Error - data is undefined");
                        return;
                    }

                    Log.green(LOG_CATEGORIES.DebugWebPlayerIssue)("data", data);

                    const maxAsString = d3.format(config.GetYAxisFormatString())(data.yDataDomain.max);
                    const minAsString = d3.format(config.GetYAxisFormatString())(data.yDataDomain.min);

                    const maxStringLength = Math.max(maxAsString.length, minAsString.length);

                    const calculatedLeftMargin = 80 + 20 * (maxStringLength / 4);
                    Log.red(LOG_CATEGORIES.DebugMarkingOffset)("calculated left margin", calculatedLeftMargin);
                    const zoommargin = !config.individualZoomSlider.value() || !isTrellis ? 40 : 0;
                    const containerSize: Size = { height: windowSize.height, width: windowSize.width - zoommargin };
                    rootContainer
            ...
            try {
                ...
                TrellisedRender(rootContainer, globalZoomSliderContainer, trellisAxisHierarchy).catch(
                    (error: Error) => {
                        Log.red(LOG_CATEGORIES.DebugDataBailout)("Caught error from TrellisedRender", error);
        ...
        addHandlersSelection((result: any) => {
            ...
            const selectionBBox: DOMRect = result.selectionDiv?.getBoundingClientRect();
            Log.green(LOG_CATEGORIES.General)(
            ...
            renderedPanels.forEach((panel) => {
                ...
                if (
                    ...
                    Log.green(LOG_CATEGORIES.General)("index marking in scope", panel.name);
                    ...
                    const x = clamp(result.x - panel.getBoundingClientRect().x, 0, panel.getBoundingClientRect().width);
                    const y = clamp(
                        result.y - panel.getBoundingClientRect().y,
                        0,
                        panel.getBoundingClientRect().bottom
                    );
                    const width = clamp(result.width, 0, panel.getBoundingClientRect().width);
                    const height = clamp(result.height, 0, panel.getBoundingClientRect().height);
